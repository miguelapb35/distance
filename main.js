class Dude {
  ...
  _drawWalkCycle() {
    ...
    if (walk_mag <= 0.05) rotate(0)
      ...
      if (this.time > max(30, (175 / walk_mag))) {
  ...
  _drawDude() {
    ...
    const look_dir = this.looking && (
      p5.Vector
        .sub(this.pos, this.look)
        .normalize()
        .mult(4)
    )

    fill(20)
    translate(18, 26); circle(-look_dir.x || 0, -look_dir.y || 0, 11)
    translate(24, 0); circle(-look_dir.x || 0, -look_dir.y || 0, 11)
...
/* static vars */
const NUM_DUDES = Math.round(innerWidth / 70), // how many 🧍‍?
...
function setup() {
  ...
  temp = document.getElementById('mask_img')
...
function draw() {
  ...
  for (let i = 0; i < NUM_DUDES; i++) {
    ...
    this.xoff = (this.xoff || random(-20, 20)) + random(-0.03, 0.03)
      this.angle = (this.angle || 0) + sin(noise(this.xoff) * TWO_PI) * random(0, 0.05)
      ...
      else if (dude.pos.x > windowWidth + WARP_MARGINS) dude.pos.x = -WARP_MARGINS
      if (dude.pos.y < -WARP_MARGINS) dude.pos.y = (windowHeight + WARP_MARGINS)
      else if (dude.pos.y > windowHeight + WARP_MARGINS) dude.pos.y = -WARP_MARGINS
